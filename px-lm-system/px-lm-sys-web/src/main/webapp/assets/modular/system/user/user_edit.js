...
layui.use(['layer', 'form', 'admin', 'laydate', 'ax', 'formSelects','layedit'], function () {
    var $ = layui.jquery;
    var $ax = layui.ax;
    var form = layui.form;
...
*/
    $.ajax({
        url:Feng.ctxPath + "/unit/list",
        type:'put',
        datatype: "json",
        success(res){
            var data = res.data;

            var pro_html = '<option value="">如是单位管理员,请选择</option>';
            for (var i in data) {
                pro_html += '<option value="' + data[i].id + '">' + data[i].name + '</option>'
            }
            $("#unitId").empty().append(pro_html);
            form.render('select')
            console.log(result);
