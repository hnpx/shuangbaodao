...
public class MerchantController extends BaseController<MerchantEntity, MerchantService> {
    ...
    @PostMapping("/update1")
    @ResponseBody
    public Object update1(ModelMap modelMap, MerchantEntity params) {
        //TODO 数据验证
        return super.update(modelMap, params, LoginContextHolder.getContext().getUserId());
    }
    ...
    public Object query(@RequestParam(required = false) Integer page,@RequestParam(required = false) Integer pageSize,
                        ...
                        @RequestParam(required = false) Integer status,@RequestParam(required = false) String contactPerson,
    ...
    public Object getminiqrQrTwo(Long id, org.springframework.ui.Model modelMap) {
           ...
           String type = "type";
           WxQr wxQr = new WxQr();
           Object object = wxQr.getminiqrQrTwo1(id, wxQr.getAccessToken(appid, secret).get("access_token").toString(), request, type, sftpHelper);
